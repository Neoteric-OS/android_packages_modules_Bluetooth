...
public:
  ...
  void ProcessGattCtpNotification(LeAudioDeviceGroup* group, uint8_t* value, uint16_t len) {
    ...
    log::debug("Ctp result OK for group {} inTransistion: {} , targetState: {}", group->group_id_,
    ...
    switch (ase->state) {
      ...
      case AseState::BTA_LE_AUDIO_ASE_STATE_DISABLING:
        log::error(
                "Ignore invalid attempt of state transition from {} to {}, {}, "
                "ase_id: {}",
                ToString(ase->state), ToString(AseState::BTA_LE_AUDIO_ASE_STATE_CODEC_CONFIGURED),
                leAudioDevice->address_, ase->id);
        group->PrintDebugState();
        break;
      case AseState::BTA_LE_AUDIO_ASE_STATE_RELEASING:
