...
static tBTM_STATUS SMP_cb(tSMP_EVT event, const RawAddress& bda,
...
static void l2test_l2c_connect_ind_cb(const RawAddress& bd_addr, uint16_t lcid,
  ...
  if (L2C_IS_VALID_COC_PSM(psm)) {
    if (psm == 200) {
      ...
      result = tL2CAP_LE_RESULT_CODE::L2CAP_LE_RESULT_NO_RESOURCES;
    } else if (psm == 201) {
      printf("L2CAP_LE_CONN_INSUFFI_AUTHORIZATION \n");
      result = tL2CAP_LE_RESULT_CODE::L2CAP_LE_RESULT_INSUFFICIENT_AUTHORIZATION;
    } else {
      result = tL2CAP_LE_RESULT_CODE::L2CAP_LE_RESULT_CONN_OK;
    }
    return;
  }
  if ((L2CAP_FCR_ERTM_MODE == g_Fcr_Mode) ||
      (L2CAP_FCR_STREAM_MODE == g_Fcr_Mode)) {
    sL2capInterface->ErtmConnectRsp(bd_addr, id, lcid, (uint16_t)tL2CAP_CONN::L2CAP_CONN_OK,
                                    (uint16_t)tL2CAP_CONN::L2CAP_CONN_OK, &t_ertm_info);
  } else {
    sL2capInterface->ConnectRsp(bd_addr, id, lcid, (uint16_t)tL2CAP_CONN::L2CAP_CONN_OK,
                                (uint16_t)tL2CAP_CONN::L2CAP_CONN_OK);
  }
  {
    tL2CAP_CFG_INFO cfg;
...
static void l2test_l2c_connect_cfm_cb(uint16_t lcid, tL2CAP_CONN result) {
...
static void l2test_l2c_config_ind_cb(uint16_t lcid, tL2CAP_CFG_INFO* p_cfg) {
  p_cfg->result = tL2CAP_CFG_RESULT::L2CAP_CFG_OK;
...
void l2test_ecfc_connect_ind_cb(const RawAddress& bdaddr,
  ...
  p_cfg.result = (tL2CAP_CFG_RESULT)result;
