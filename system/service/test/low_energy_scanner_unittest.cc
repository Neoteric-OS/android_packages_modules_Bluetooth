...
public:
  ...
  MOCK_METHOD2(RegisterScanner,
               void(const bluetooth::Uuid& app_uuid, BleScannerInterface::RegisterCallback));
  ...
  MOCK_METHOD5(ScanFilterParamSetupImpl,
               void(uint8_t client_if, uint8_t action, uint8_t filt_index,
                    btgatt_filt_param_setup_t* filt_param, FilterParamSetupCallback cb));
  ...
  MOCK_METHOD4(SetScanParameters,
               void(int scanner_id, int scan_interval, int scan_window, Callback cb));

  MOCK_METHOD5(BatchscanConfigStorage,
               void(int client_if, int batch_scan_full_max, int batch_scan_trunc_max,
                    int batch_scan_notify_threshold, Callback cb));

  MOCK_METHOD6(BatchscanEnable, void(int scan_mode, int scan_interval, int scan_window,
  ...
  MOCK_METHOD8(StartSync, void(uint8_t, RawAddress, uint16_t, uint16_t, StartSyncCb, SyncReportCb,
                               SyncLostCb, BigInfoReportCb));
  ...
  MOCK_METHOD5(SyncTxParameters, void(RawAddress, uint8_t, uint16_t, uint16_t, int));
  ...
  void ScanFilterParamSetup(uint8_t client_if, uint8_t action, uint8_t filt_index,
                            std::unique_ptr<btgatt_filt_param_setup_t> filt_param,
                            FilterParamSetupCallback cb) override {
    ScanFilterParamSetupImpl(client_if, action, filt_index, filt_param.get(), std::move(cb));
  }
...
public:
  ...
  void OnScanResult(LowEnergyScanner* scanner, const ScanResult& scan_result) override {
    ASSERT_TRUE(scanner);
    scan_result_count_++;
...
public:
  ...
  void SetUp() override {
    ...
    if (!mock_handler_) {
      mock_handler_.reset(new MockScannerHandler());
    }
    fake_hal_gatt_iface_ = new hal::FakeBluetoothGattInterface(
            nullptr, std::static_pointer_cast<BleScannerInterface>(mock_handler_), nullptr,
            nullptr);
...
public:
  ...
  LowEnergyScannerPostRegisterTest(const LowEnergyScannerPostRegisterTest&) = delete;
  ...
  void RegisterTestScanner(
          const std::function<void(std::unique_ptr<LowEnergyScanner> scanner)> callback) {
...
TEST_F(LowEnergyScannerTest, RegisterInstance) {
  ...
  auto callback = [&](BLEStatus in_status, const Uuid& uuid,
    ...
    cb_uuid = uuid;
    scanner =
            std::unique_ptr<LowEnergyScanner>(static_cast<LowEnergyScanner*>(in_scanner.release()));
    callback_count++;
  };
  ...
  EXPECT_CALL(*mock_handler_, Unregister(scanner_if0)).Times(1).WillOnce(Return());
  scanner.reset();
  ::testing::Mock::VerifyAndClearExpectations(mock_handler_.get());
...
TEST_F(LowEnergyScannerPostRegisterTest, ScanSettings) {
  EXPECT_CALL(mock_adapter_, IsEnabled()).WillOnce(Return(false)).WillRepeatedly(Return(true));
...
TEST_F(LowEnergyScannerPostRegisterTest, ScanRecord) {
  ...
  fake_hal_gatt_iface_->NotifyScanResultCallback(kTestAddress, kTestRssi, kTestRecord0);
  EXPECT_EQ(0, delegate.scan_result_count());

  // Start a scan session for |le_scanner_|.
  EXPECT_CALL(mock_adapter_, IsEnabled()).Times(1).WillOnce(Return(true));
  EXPECT_CALL(*mock_handler_, Scan(_)).Times(2).WillOnce(Return()).WillOnce(Return());
  ScanSettings settings;
  std::vector<ScanFilter> filters;
  ...
  fake_hal_gatt_iface_->NotifyScanResultCallback(kTestAddress, kTestRssi, kTestRecord1);
  EXPECT_EQ(2, delegate.scan_result_count());
  EXPECT_EQ(kTestAddressStr, delegate.last_scan_result().device_address());
  EXPECT_EQ(kTestRssi, delegate.last_scan_result().rssi());
  EXPECT_TRUE(delegate.last_scan_result().scan_record().empty());

  fake_hal_gatt_iface_->NotifyScanResultCallback(kTestAddress, kTestRssi, kTestRecord2);
