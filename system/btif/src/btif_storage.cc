...
void btif_storage_set_encr_data_cccd(const RawAddress& bd_addr, uint8_t cccd) {
  do_in_jni_thread(Bind(
          [](const RawAddress& bd_addr, uint8_t cccd) {
            btif_config_set_int(bd_addr.ToString(), BTIF_STORAGE_KEY_ENCR_DATA_CCCD, cccd);
          },
          bd_addr, cccd));
}
...
uint8_t btif_storage_get_encr_data_cccd(const RawAddress& bda) {
  ...
  btif_config_get_int(bda.ToString(), BTIF_STORAGE_KEY_ENCR_DATA_CCCD, &cccd);
  return cccd;
}
 ...
 *
 ******************************************************************************/
void btif_storage_set_enc_key_material(const RawAddress& remote_bd_addr, uint8_t* value,
                                       size_t key_length) {
  do_in_jni_thread(Bind(
          [](const RawAddress& remote_bd_addr, uint8_t* value, uint8_t key_length) {
            btif_config_set_bin(remote_bd_addr.ToString(), BTIF_STORAGE_KEY_ENCR_DATA, value,
                                key_length);
          },
          remote_bd_addr, value, key_length));
}
 ...
 *
 * Description      Finds the value of the encrypted data key material
 *                  is present or not?
 *
 * Returns          BT_STATUS_SUCCESS if the fetch was successful,
 *                  BT_STATUS_FAIL otherwise
...
bt_status_t btif_storage_get_enc_key_material(const RawAddress* remote_bd_addr, uint8_t* key_value,
                                              size_t* key_length) {
  std::string key =
          (remote_bd_addr == NULL) ? BTIF_STORAGE_SECTION_ADAPTER : remote_bd_addr->ToString();
  return btif_config_get_bin(key, BTIF_STORAGE_KEY_ENCR_DATA, key_value, key_length)
                 ? BT_STATUS_SUCCESS
                 : BT_STATUS_FAIL;
}
...
/*******************************************************************************
 *
 * Function         btif_storage_get_enc_key_material_length
 *
 * Description      Finds the length of the value of the
 *                  encrypted data key material
 *
 * Returns          Returns the length of the encrypted data key material
 *
 ******************************************************************************/
