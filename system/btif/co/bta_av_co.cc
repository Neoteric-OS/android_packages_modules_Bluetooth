      ...
      if (p_peer->GetCodecs()->setCodecConfig(p_sink->codec_caps, true, new_codec_config, true)) {
        ...
        if (bluetooth::audio::a2dp::is_hal_enabled()) {
          ...
          bluetooth::audio::a2dp::setup_codec(
                  a2dp_codec_config, peer_mtu,
                  encoder == nullptr ? 0 : encoder->get_encoder_interval_ms() * 1000);
        }
      }
...
const BtaAvCoSep* BtaAvCo::SelectSourceCodec(BtaAvCoPeer* p_peer) {
  ...
  for (const auto& iter : p_peer->GetCodecs()->orderedSourceCodecs()) {
    ...
    if (!p_peer->GetCodecs()->setCodecConfig(sink_codec_cap, true /* is_capability */,
...
void bta_av_co_aptx_ad_support(std::vector<btav_a2dp_codec_config_t> offloading_preference) {
  ...
  A2DP_SetAptxADSupport(offloading_preference, soc_add_on_features_len,
    ...
    BTM_SPLIT_A2DP_SOURCE_APTX__ADAPTIVE_SUPPORTED(soc_add_on_features->as_array));
}
