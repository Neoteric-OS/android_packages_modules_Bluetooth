...
void btm_ble_process_ext_adv_pkt(uint8_t data_len, const uint8_t* data) {
  ...
  uint8_t addr_type, num_reports, pkt_data_len, primary_phy, secondary_phy, advertising_sid;
  ...
  while (num_reports--) {
    ...
    btm_ble_process_adv_pkt_cont(event_type, addr_type, bda, primary_phy, secondary_phy,
                                 advertising_sid, tx_power, rssi, periodic_adv_int, pkt_data_len,
...
void btm_ble_process_adv_pkt(uint8_t data_len, const uint8_t* data) {
  ...
  if (data_len < bytes_to_process) {
    log::error("Malformed LE advertising packet: not enough room for num reports");
    return;
  }
  ...
  while (num_reports--) {
    ...
    if (data_len < bytes_to_process) {
      log::error("Malformed LE advertising packet: not enough room for metadata");
      return;
    }
    ...
    if (legacy_evt_type == BTM_BLE_ADV_IND_EVT) {
      event_type |= (1 << BLE_EVT_CONNECTABLE_BIT) | (1 << BLE_EVT_SCANNABLE_BIT);
    ...
    btm_ble_process_adv_pkt_cont(event_type, addr_type, bda, PHY_LE_1M, PHY_LE_NO_PACKET,
                                 NO_ADI_PRESENT, TX_POWER_NOT_PRESENT, rssi,
                                 0x00 /* no periodic adv */, pkt_data_len, pkt_data, original_bda);
  }
}
