...
class InterfaceImpl : public Interface {
  ...
  void SendVendorSpecificCmd(unsigned short opcode, unsigned char len, unsigned char* param,
                             void (*cb)(tBTM_VSC_CMPL*)) const override {
...
void btsnd_hcic_flow_spec(uint16_t handle, uint8_t unused, uint8_t direction, uint8_t service_type,
                          uint32_t token_rate, uint32_t token_size, uint32_t peak,
                          uint32_t latency) {
  BT_HDR* p = (BT_HDR*)osi_malloc(HCI_CMD_BUF_SIZE);
  uint8_t* pp = (uint8_t*)(p + 1);
