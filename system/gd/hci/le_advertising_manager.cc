...
struct LeAdvertisingManager::impl : public bluetooth::hci::LeAddressManagerCallback {
  ...
  void reset_advertiser(AdvertiserId id) {
    ...
    if (advertising_api_type_ == AdvertisingApiType::EXTENDED) {
      ...
      if (advertising_sets_[id].address_rotation_wake_alarm_ != nullptr) {
        advertising_sets_[id].address_rotation_wake_alarm_->Cancel();
        advertising_sets_[id].address_rotation_wake_alarm_.reset();
      }
    }
